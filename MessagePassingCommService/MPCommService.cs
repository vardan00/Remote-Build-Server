...
namespace MessagePassingComm
{
    ...
    public class Sender
    {
        ...
        {
            port = listenPort;
            fromAddress = baseAddress + listenPort.ToString() + "/MessagePassingComm";
        ...
        {
            toAddress = baseAddress + ":" + port.ToString() + "/MessagePassingComm";
        ...
        public bool postFile(string fileName, string destinationIPAddress, string destinationAddress)
        {
            FileStream fs = null;
            long bytesRemaining;
            try
            {
                createSendChannel(destinationIPAddress);
                string path = fileName;
                fs = File.OpenRead(path);
                bytesRemaining = fs.Length;
                channel.openFileForWrite(destinationAddress + "/" + Path.GetFileName(fileName));
                while (true)
                {
    ...
    public class Comm
    {
        ...
        public bool postFile(string filename, string destinationIPAddress, string destinationAddress)
        {
            return sndr.postFile(filename, destinationIPAddress, destinationAddress);
        }
